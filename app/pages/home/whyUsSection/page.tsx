...
export default function WhyUsSection(): JSX.Element {
  const features: Feature[] = [
    {
      ...
      icon: <Layers size={60} strokeWidth={1} className="text-white/60" />,
    },
    {
      title: 'Precision Planning',
      description: 'We map strengths and weaknesses, preparing for what\'s next.',
      icon: <BarChart3 size={60} strokeWidth={1} className="text-white/60" />,
    },
    {
      title: 'Passionate Advocacy',
      description: 'We personalize strategies for unique client needs.',
      icon: <Gauge size={60} strokeWidth={1} className="text-white/60" />,
    },
    {
      title: 'Tailored Approach',
      description: 'We personalize strategies for unique client needs.',
      icon: <CheckCheck size={60} strokeWidth={1} className="text-white/60" />,
    },
    {
      title: 'Result-driven Execution',
      description: 'We focus on delivering positive outcomes with precision.',
      icon: <Target size={60} strokeWidth={1} className="text-white/60" />,
    },
    {
      title: 'Unwavering Support',
      description: 'We\'re with you every step of the way, providing guidance and support.',
      icon: <ArrowRight size={60} strokeWidth={1} className="text-white/60" />,
    },
  ];
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-black via-[#0a0e27] to-[#1a2357] text-white flex flex-col items-center justify-center px-16 py-24">
      <div className="max-w-[1400px] w-full">
        ...
        >
          <h2 className="text-6xl md:text-7xl font-light tracking-wide mb-8">
            Why Us?
          </h2>
        ...
        >
          {features.map((feature: Feature, index: number) => (
            <motion.div
              key={index}
              className="group relative bg-[#0d1133]/50 border border-white/20 rounded-3xl p-10 flex items-start justify-between hover:bg-[#0d1133]/70 hover:border-white/30 transition-all duration-300 min-h-[180px]"
            ...
            >
              ...
              <div className="flex-1 pr-6">
                <h3 className="text-2xl font-light italic text-white mb-4 tracking-wide">
                  {feature.title}
                </h3>
